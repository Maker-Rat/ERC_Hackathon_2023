...
class RRT():
    def __init__(self):
        ...
        self.parent_dict[start_point] = start_point
    ...
    def plan_path_RRT(self):
        ...
        while not path_found:
            ...
            if count!=0:
                start = targets[count-1]
            else:
                start = (-5.06,-3.12)

            self.point_list = [start]

            while not found:
                
                p = (uniform(-6, 6), uniform(-3, 3))
                dist, nearest = min_dist(p, self.point_list)
                if (dist <= 0.5) and (not obsdet.isPointInObstacles(p[0], p[1])) and (obsdet.isValidPoint(nearest[0], nearest[1], p[0], p[1])):
                    self.parent_dict[p] = nearest
                    self.point_list.append(p)
                    if sqrt((target[0]-p[0])**2 + (target[1]-p[1])**2) <= 0.5 and obsdet.isValidPoint(target[0], target[1], p[0], p[1]):
                        self.parent_dict[target] = p
                        ...
                        pnt = target
                        while pnt != start :
                            if (self.parent_dict[pnt] == pnt):
                                break
                            temp_path.append(pnt)
                            pnt = self.parent_dict[pnt]
...
path_planner = RRT()
#path_planner.PRM()
path = path_planner.plan_path_RRT()
